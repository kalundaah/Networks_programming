...
void handle_client(int server_fd, struct sockaddr_in *client_address, socklen_t client_address_length);
...
int main(int argc, char const *argv[])
{
    ...
    while (1)
    {
        ...
        // ssize_t bytes_received = recvfrom(server_fd, buffer, BUFFER_SIZE, 0, (struct sockaddr *)&client_address, &client_address_length);
        // if (bytes_received < 0)
        // {
        ...
        {
            // Child process
            handle_client(server_fd, &client_address, client_address_length);
            exit(0);
        }
        else if (pid < 0)
        {
