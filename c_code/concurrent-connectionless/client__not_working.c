...
void handle_server_response(int client_fd, struct sockaddr_in *server_address, socklen_t server_address_length);
...
int main(int argc, char const *argv[])
{
    ...
    checkMemoryAllocation(SERVER_HOSTNAME);
    ...
    // Fork to handle server responses
    pid_t pid = fork();
    if (pid == 0)
    {
        // Child process to handle server responses
        handle_server_response(client_fd, &server_address, server_address_length);
        exit(0);
    }
    ...
    while (1)
    {
        ...
        // send to server
        if (sendto(client_fd, buffer, BUFFER_SIZE, 0, (struct sockaddr *)&server_address, server_address_length) < 0)
        {
...
void handle_server_response(int client_fd, struct sockaddr_in *server_address, socklen_t server_address_length)
{
    ...
    {
        bzero(buffer, BUFFER_SIZE);
        ssize_t bytes_received = recvfrom(client_fd, buffer, BUFFER_SIZE, 0, (struct sockaddr *)server_address, &server_address_length);
        if (bytes_received < 0)
        {
